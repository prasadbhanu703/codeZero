
const Part3 = () => {
    return(<div >
        <div className="flex_container" style = {{ boxShadow: "5px 5px 10px 10px rgb(220, 221, 221)", borderRadius:"5px"}}>
            <div >
            <img style={{height:"500px"}}  src="https://miro.medium.com/max/1600/0*gMNo1pAG3bJHZZlM.jpg" alt="not supported" />
            </div>
            <div>
                <img src="data:image/png;base64," alt="doesnt supports" />
                <h3>"Landkit is hands down the  most useful front end Bootsrap theme I've ever used. I can't wait to use it again for my next project. "</h3>
                <h5> DAVE GAMACHE</h5>
                </div>
            </div>
            <div className="flex_container" style={{marginTop:"40px"}}>
                <div>
                    <h2>Stay focused on your business.</h2>
                    <h2 style={{color:"blue"}}>Let us handle the design.</h2>
                    <p>You have a business to run. Stop worring about cross-browser bugs, designing new pages, keeping your components up to date. Let us do that for you.</p>
                    <div className="flex_container">
                       <div> <h5>100%</h5>
                            <p>Satisfaction</p>
                       </div>
                       <div> <h5>24/7</h5>
                            <p>Support</p>
                       </div>
                       <div> <h5>100k+</h5>
                            <p>Sales</p>
                       </div>
                    </div>
                </div>
                <div>
                    <img style={{height:"450px"}} src="https://smart.macinteract.com/wp-content/uploads/2021/04/illustration-8.png" alt="doesn't support"  />
                </div>
            </div>
            <h1> Fair, simple pricing for all. </h1>
            <h3><p> All types of businesses need access to development resources, so we give you the option to decide how much you need to use. </p></h3>
            <span style={{color:"grey"}}>Annual</span> <span style={{color:"grey"}} >Monthly</span>
    </div>
    )
}

export default Part3;